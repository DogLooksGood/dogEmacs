...
(defvar user/count-same-key 1)
(defvar user/upcase-trigger-count 3)
(defvar user/upcase-delay-interval 0.225)
(defvar user/upcase-repeat-interval 0.05)
(defvar user/upcase-mc-count 1)

(setq user/upcase-delay-interval 0.182
      user/upcase-repeat-interval 0.05)
    ...
    (if (and user/last-self-insert-char
             ...
             (time-less-p (current-time)
                          (time-add user/last-self-insert-time
                                    (if (> user/count-same-key user/upcase-mc-count)
                                        user/upcase-repeat-interval
                                      user/upcase-delay-interval))))
        (let* ((cnt-max (* mc-count user/upcase-trigger-count))
